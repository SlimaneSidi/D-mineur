            ...
            for (int dy = -1; dy <= 1; dy++) {
                for (int dx = -1; dx <= 1; dx++) {
                    if (dx != 0 || dy != 0) {
                        int nx = x + dx;
                        int ny = y + dy;
                        if (nx >= 0 && nx < grille->largeur && ny >= 0 && ny < grille->hauteur && !grille->cases[ny * grille->largeur + nx].cellRevealed) {
    ...
    for (int dy = -1; dy <= 1; dy++) {
        for (int dx = -1; dx <= 1; dx++) {
            ...
            if (nx >= 0 && nx < grille->largeur && ny >= 0 && ny < grille->hauteur) {
                if (grille->cases[ny * grille->largeur + nx].isMine) {
                    count++;
                }
            }
        }
